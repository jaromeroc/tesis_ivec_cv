...
# Marco teórico: enfoques y herramientas para el análisis de la vulnerabilidad social
...
### El modelo PAR: causalidad estructural y condiciones inseguras

El modelo PAR (Pressure and Release), desarrollado por @blaikie1994, constituye una de las propuestas más influyentes en la crítica a la visión tecnocrática y despolitizada de los desastres.
...
### El marco MOVE: multidimensionalidad, integración y contexto

El marco MOVE (*Methods for the Improvement of Vulnerability Assessment in Europe*), surgido en el contexto del proyecto europeo del mismo nombre [@birkmann2013], constituye uno de los esfuerzos más sistemáticos por articular una lectura multidimensional, contextual y operativa de la vulnerabilidad.
...
## El enfoque funcional de Cardona: operacionalización del riesgo y capacidades

El enfoque funcional propuesto por Cardona [@cardona2001; @cardona2003; @cardona2014] representa una contribución clave en el campo de la evaluación del riesgo, especialmente en el ámbito latinoamericano.
...
## Articulación crítica de enfoques: hacia una comprensión integrada de la vulnerabilidad
...
## Convergencia metodológica hacia el modelo IVEC
...
```{r ivec-flujo, echo=FALSE, fig.align='center', fig.cap="Arquitectura epistemotécnica del modelo IVEC y su extensión hacia el riesgo  \\newline \\textit{Nota.} Las capacidades reactivas (CR) se activan en situaciones de alerta o en escenarios prospectivos. \\newline \\textit{Fuente:} Elaboración propia.", fig.scap="Arquitectura epistemotécnica del modelo IVEC", message=FALSE, warning=FALSE, out.width="100%"}
...
# --- Paleta armónica IVEC–Soft Earth ---
col_epistemologia   <- "#8BA9C6"  # azul grisáceo suave
col_teoria          <- "#A7BE84"  # verde oliva claro
col_modelo          <- "#C7A27C"  # marrón arena
col_indicadores     <- "#E2CFA4"  # beige claro
col_operacional     <- "#F2E2C4"  # crema cálido
col_indices         <- "#B4C8E1"  # azul perlado
col_riesgo          <- "#E6B0A5"  # coral pálido

grafo <- grViz(glue::glue("
digraph IVEC_Flujo {{
  graph [layout = dot, rankdir = TB, ranksep=0.8, bgcolor = '#FFFFFF']
  node [shape = ellipse, style = filled, fontname = 'serif', fontsize = 16, width = 2, color = '#444444']

  // --- Nivel 1: Epistemologías ---
  EpCritica     [label='Epistemología\\ncrítica', fillcolor='{col_epistemologia}']
  EpSistemas    [label='Epistemología\\nde sistemas', fillcolor='{col_epistemologia}']

  // --- Nivel 2: Teorías ---
  TeoriaRel     [label='Teoría relacional\\nestructural', fillcolor='{col_teoria}']
  TeoriaSoc     [label='Teoría\\nsocioecológica', fillcolor='{col_teoria}']

  // --- Nivel 3: Modelos ---
  ModeloPAR     [label='Modelo PAR \\n1994', fillcolor='{col_modelo}']
  ModeloMOVE    [label='Modelo MOVE\\n2012', fillcolor='{col_modelo}']
  ModeloCardona [label='Modelo Holístico\\nde Riesgo\\n2001', fillcolor='{col_modelo}']

  // --- Nivel 4: Indicadores ---
  IndicadoresPAR     [label='Indicadores PAR:\\nVE, CAD', fillcolor='{col_indicadores}']
  IndicadoresMOVE    [label='Indicadores MOVE:\\nVi, VC, CH, CS', fillcolor='{col_indicadores}']
  IndicadoresCardona [label='Indicadores Cardona:\\n(CR)', fillcolor='{col_indicadores}']

  // --- Nivel 5: Operacionalización ---
  Operacionalizacion [label='Operacionalización\\nmetodológica', fillcolor='{col_operacional}']

  // --- Nivel 6: Índices IVEC ---
  IndiceIVEC [label='Índice IVEC', fillcolor='{col_indices}', penwidth=2]
  IndiceIVEC_Riesgo [label='Índice IVEC_Riesgo (CR)', fillcolor='{col_riesgo}', penwidth=2]

  // --- Conexiones epistemológicas ---
  EpCritica  -> TeoriaRel
  EpSistemas -> TeoriaSoc
  TeoriaRel  -> ModeloPAR
  TeoriaSoc  -> ModeloMOVE
  TeoriaRel  -> ModeloCardona [style=dashed, label='influencia teórica']
  EpSistemas -> ModeloCardona [style=dashed]

  // --- Conexiones entre modelos e indicadores ---
  ModeloPAR -> IndicadoresPAR [style=dashed]
  ModeloMOVE -> IndicadoresMOVE
  ModeloCardona -> IndicadoresCardona

  // --- Integración de indicadores ---
  IndicadoresPAR -> Operacionalizacion [style=dashed]
  IndicadoresMOVE -> Operacionalizacion
  IndicadoresCardona -> Operacionalizacion

  // --- Núcleo y salidas ---
  Operacionalizacion -> IndiceIVEC

  Vulnerabilidad [label='Vulnerabilidad social = ', shape=none, color=transparent, style=none, fontname='serif', fontsize=15]
  Vulnerabilidad -> IndiceIVEC [style=invis]
  {{ rank = same; Vulnerabilidad; IndiceIVEC; }}
  IndiceIVEC -> IndiceIVEC_Riesgo

  // --- Nivel 7: interacción del riesgo ---
  Riesgo [label='Riesgo = ', shape=none, color=transparent, style=none, fontname='serif', fontsize=15]
  Peligrosidad [label='Peligrosidad (H)', fillcolor='{col_riesgo}']
  Exposición [label='Exposición (E)', fillcolor='{col_riesgo}']

  Riesgo -> Peligrosidad [style=invis]
  Peligrosidad -> IndiceIVEC_Riesgo [dir=none]
  IndiceIVEC_Riesgo -> Exposición [dir=none]

  {{ rank = same; Riesgo; Peligrosidad; IndiceIVEC_Riesgo; Exposición; }}
}}
"))

# --- Exportar figuras ---
DiagrammeRsvg::export_svg(grafo) %>%
  charToRaw() %>%
  rsvg::rsvg_png("figures/IVEC_marco_teorico_soft.png")

DiagrammeRsvg::export_svg(grafo) %>%
  charToRaw() %>%
  rsvg::rsvg_pdf("figures/IVEC_marco_teorico_soft.pdf")

knitr::include_graphics("figures/IVEC_marco_teorico_soft.png")
```
